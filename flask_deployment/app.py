...
try:
    ...
    print("Please run 'python retrain_models.py' to regenerate models")
    model_hotel = None
    features_hotel = None
...
try:
    ...
    print(f"Error loading flight cancellation model: {e}")
    print("Please run 'python retrain_models.py' to regenerate models")
    model = None
    scaler = None
...
def hotel1_home():
    ...
    if model_hotel is None:
        return "Error: Hotel price model not loaded. Please retrain the model first."
...
@app.route('/api/products/<int:product_id>/forecast', methods=['GET'])
def get_product_forecast(product_id):
    conn = get_connection()
    df = pd.read_sql("SELECT * FROM all_products WHERE id = %s", conn, params=(product_id,))
    conn.close()
    ...
    return jsonify({
        'history': subset.to_dict('records'),
